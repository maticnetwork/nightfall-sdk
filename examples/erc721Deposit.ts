import { UserFactory } from "../libs/user";
import { config } from "./appConfig";
// import ERC721Abi from "../libs/tokens/abis/ERC721.json";
import Web3 from "web3";
import { from } from "form-data";
import { Console } from "console";

const ERC721Abi = require("../libs/tokens/abis/ERC721.json");

const web3 = new Web3(config.blockchainWsUrl);

// Example script
const main = async () => {
  let user;
  let latestTokenId;

  let erc721Mock = new web3.eth.Contract(
    ERC721Abi,
    "0x00269d82110435a4e7043d53e5bf79a8bd767b17",
  );

  try {
    // # 1 Create an instance of User (mnemonic is optional)
    user = await UserFactory.create({
      blockchainWsUrl: config.blockchainWsUrl,
      clientApiUrl: config.clientApiUrl,
      ethereumPrivateKey: config.ethereumPrivateKey,
      nightfallMnemonic: config.nightfallMnemonic,
    });
    const contract = new web3.eth.Contract(ERC721Abi, config.erc721TestAddress);

    const accs = await web3.eth.getAccounts();

    console.log("accs", accs);

    //number of tokens owned by the address
    const balance = await contract.methods
      .balanceOf(config.ethereumPublicKey)
      .call();

    console.log("Balance", balance);

    const tx = await contract.methods.awardItem(
      config.ethereumPublicKey,
      "test",
    );
    const gas = await tx.estimateGas({
      from: config.ethereumPublicKey,
    });

    const gasPrice = await web3.eth.getGasPrice();
    const data = tx.encodeABI();
    const nonce = await web3.eth.getTransactionCount(config.ethereumPublicKey);
    const signedTx = await web3.eth.accounts.signTransaction(
      {
        to: config.erc721TestAddress,
        data,
        gas,
        gasPrice,
        nonce,
      },
      config.ethereumPrivateKey,
    );

    const receipt = await web3.eth.sendSignedTransaction(
      signedTx.rawTransaction,
      function (error, hash) {
        if (!error) {
          console.log(hash);
        } else {
          console.log(error);
        }
      },
    );

    latestTokenId = await contract
      .getPastEvents("Transfer", {
        filter: {
          _from: config.ethereumPublicKey,
        },
        fromBlock: 0,
      })
      .then((events) => {
        latestTokenId = events[events.length - 1].returnValues.tokenId;
        console.log("myid", latestTokenId);
        return events[events.length - 1].returnValues.tokenId;
      });

    // # 4 Make ERC721 deposit
    const tokenContractAddress = config.erc721TestAddress;
    const tokenErcStandard = "ERC721";
    const value = "0";
    const tokenId = latestTokenId;

    // add only one to be required, not both value and tokenId
    const txReceipts = await user.makeDeposit({
      tokenContractAddress,
      tokenErcStandard,
      value,
      tokenId,
    });

    console.log("Transaction receipts", txReceipts);

    // # 5 [OPTIONAL] You can check the transaction hash
    console.log("Nightfall deposit tx hashes", user.nightfallDepositTxHashes);

    // # 6 [OPTIONAL] You can check deposits that are not yet in Nightfall
    const pendingDeposits = await user.checkPendingDeposits();
    console.log("Pending balances", pendingDeposits);
  } catch (error) {
    console.log(error);
    process.exit(1);
  } finally {
    user.close();
    console.log("Bye bye");
  }
};

main();
